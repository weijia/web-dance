...
export class MotionTracker {
  ...
  /**
   * 初始化MediaPipe HandLandmarker (新版 tasks-vision API)
   */
  private async initializeMediaPipe(): Promise<void> {
    try {
      // 1. 初始化 FilesetResolver
      const vision = await FilesetResolver.forVisionTasks(
        'https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision/wasm'
      );

      // 2. 创建 HandLandmarker 实例
      this.handLandmarker = await HandLandmarker.createFromModelPath(
        vision,
      ...
      console.error('MediaPipe HandLandmarker 初始化失败:', error);
      throw error;
    }
  }

  /**
   * 启动检测循环（使用 requestAnimationFrame）
   */
  private startDetectionLoop(): void {
    const loop = async () => {
      if (!this.state.isTracking || !this.video || !this.handLandmarker) {
      ...
      if (now - this.lastProcessTime >= this.processingInterval && !this.isProcessing) {
        ...
        try {
          // 新版 API: detectForVideo(videoElement, timestamp)
  ...
  /**
   * 处理MediaPipe结果（新版 API）
   * @param result HandLandmarkerResult
   */
  private processResults(result: HandLandmarkerResult): void {
    // 新版 API: result.landmarks 是二维数组 [handIndex][landmarkIndex]
  ...
  private drawHandLandmarks(landmarks: any[], handedness: string): void {
    ...
    // 绘制连接线（新版 API 使用 HandLandmarker.HAND_CONNECTIONS）
    this.ctx.beginPath();
  ...
  dispose(): void {
    ...
    if (this.handLandmarker) {
      console.log('MotionTracker.dispose: 清理 HandLandmarker');
      try {
        this.handLandmarker.close();
      } catch (e) {
        console.error('关闭 HandLandmarker 时出错:', e);
      }
